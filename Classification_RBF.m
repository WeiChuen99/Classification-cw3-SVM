...
for i = 1:10
    ...
    fprintf("Sigma Value: %d, constraint: %d\n",currentModelParameters.qValue,currentModelParameters.cValue);
    ...
    currentModel = fitcsvm(trainingInputs, trainingTargets, 'Standardize', true, 'KernelFunction', 'rbf', 'KernelScale', currentModelParameters.qValue, 'BoxConstraint', currentModelParameters.cValue);
...
function [bestHyperParameter] = innerLoop(inputs,targets,k)
    ...
    numOfExamples = (k-1)*size(inputs,1)/k; % numOfExamples = 4500*4/5
    foldLength = size(inputs,1)/k;          % foldLength = 4500/5
    ...
    for qIndex=1:length(sigmaValues)

        for cIndex=1:length(cValues)

            fprintf("InnerLoop c:value: %d, Sigma Value : %d\n",cValues(cIndex),sigmaValues(qIndex));
            f1s = zeros(1,k);
            recalls = zeros(1,k);
            ...
            for i=1:k     % each iteration performs one time of training and CV
                ...
                SVM = fitcsvm(trainingInputs, trainingTargets, 'Standardize', true, 'KernelFunction', 'rbf', 'KernelScale', sigmaValues(qIndex), 'BoxConstraint', cValues(cIndex));
            ...
            struct.qValue = sigmaValues(qIndex);
